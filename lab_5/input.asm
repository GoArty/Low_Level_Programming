...
string    rb 81             ;80 + 1 байт для символа конца строки '$'
handle    rw 1              ;Дескриптор файла
max_len dw 0

char_first  db 'f' 		; символ, который заменяем
char_second db 'b' 		; указанный новый символ
size_ 	    db 0
;-------------------------------------------------------------------------------
;-------------------------------------------------------------------------------
...
@@: mov [handle],ax         ;Сохранение дескриптора файла
    ...
    mov dx, string           ;Адрес буфера с данными
    movzx cx,[size]         ;Размер данных
    int 21h                 ;Обращение к функции DOS
    jnc close_file_2          ;Если нет ошибки, то закрыть файл
    call error_msg          ;Вывод сообщения об ошибке
    ...
    close_file_2:
        ...
        mov bx,[handle]         ;Дескриптор
        int 21h                 ;Обращение к функции DOS
        jnc exit_2                ;Если нет ошибки, то выход из программы
        call error_msg          ;Вывод сообщения об ошибке
